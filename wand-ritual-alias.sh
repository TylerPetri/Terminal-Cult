...
for i in {1..3}; do
  toilet -f future --gay "Invoking..." | lolcat
  pause 0.5
done
...
if command -v cowsay &>/dev/null; then
  echo "The daemon nods in approval." | cowsay | lolcat
fi
...
# Easter egg: summon cult handle
if [[ "${1:-}" == "--alias" ]]; then
  ADJ=(ghost sigil core ptrace null fork seg panic hex mount init loop bg daemon wand sync sig)
  NOUN=(ritual glitch punk .exe wizard ghost 404 boy bomb spawn whisper dancer veil fuzz curse trace dump shock script seer shard flame)

  a=${ADJ[$RANDOM % ${#ADJ[@]}]}
  b=${NOUN[$RANDOM % ${#NOUN[@]}]}
  ALIAS="$a$b"

  echo -e "\n🔮 Your Terminal Cult alias is:\n"
  sleep 0.4
  figlet -f slant "$ALIAS" | lolcat
  echo -e "\n# You are the spell."
  exit 0
fi